...
def process_all_frame(self, src, class_names):
    frame = src.copy()
    ...
    for i in range(box_count[0]):
        ...
        if i in violate:
            ...
            if self.show_score:
                cv2.putText(
                    frame,
                    f"Distance violation - {label} [{score}%]",
...
def detect_all(self):
    ...
    if self.source == "Video" or self.source == "Cam":
        ...
        while True:
            ...
            self.process_all_frame(self, src, class_names)
        ...
        self.process_all_frame(self, frame, class_names)